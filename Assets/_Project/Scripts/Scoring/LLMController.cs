...
public class LLMController : MonoBehaviour
{
    ...
    public void EvaluateResidentialStars()
    {
...
//        string llmResponse = GetLLMResponseWithTimeoutAsync(prompt, 5000).Result;
        if (llmResponse == null)
        {
        ...
        int residentialStars = ParseResidentialStars(llmResponse);
        // bool hasHiddenStar = ParseResidentialHiddenStar(llmResponse);

        Debug.Log($"Residential faction stars: {residentialStars}");
        // if (hasHiddenStar)
        // {
        //     Debug.Log("Residential faction awarded a hidden 5th star!");
        // }
    }
    ...
    //     try 
    //     {
    //         return await _llmService.GetLLMResponseAsync(prompt, _cts.Token);
    //     }
