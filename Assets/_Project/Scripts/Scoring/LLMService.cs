...
public class LLMService : MonoBehaviour
{
    ...
    private void Awake()
    {
        apiKey = "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";
    ...
    private string GetSystemPrompt(string floodType)
    {
        if (string.IsNullOrEmpty(floodType))
            throw new ArgumentException("Flood type cannot be empty");

        return $"You are an educational assistant specializing in {floodType} flooding. " +
               "Provide clear, accurate, and age-appropriate explanations about flood-related topics. " +
               "Keep responses concise (maximum 3-4 sentences) and focus on practical understanding. " +
               "If asked about emergency situations, always emphasize safety and following official guidance.";
    }

    public async Task<string> AskQuestion(string question, string floodType, CancellationToken cancellationToken = default)
    {
        // Input validation
        ...
        while (retryCount <= config.maxRetries)
        {
            try
            {
                return await SendRequest(question, floodType, cancellationToken);
            }
    ...
    private async Task<string> SendRequest(string question, string floodType, CancellationToken cancellationToken)
    {
        var messages = new Message[]
        {
            new Message("system", GetSystemPrompt(floodType)),
