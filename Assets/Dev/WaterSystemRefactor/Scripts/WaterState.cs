...
/// supplied by MapAccessor; this object owns only live simulation values.
/// </summary>
public sealed class WaterState
{
    ...
    public WaterState(MapAccessor map)
    {
        _map = map;
    ...
    internal WaterState Clone()
    {
        WaterState clone = new WaterState(_map);
    ...
    private void LoadMapSnapshot()
    {
        ...
        for (int y = 1; y <= Height; y++)
        {
            for (int x = 1; x <= Width; x++)
            {
                if (!_map.TryGetCell(x, y, out MapCellDef cell))
                    continue;
