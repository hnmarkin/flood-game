...
/// </summary>
public sealed class WaterPhysicsBarrier
{
    ...
    public bool InitializeForSimulation(int gridWidth, int gridHeight)
    {
        ...
        {
            Debug.LogError("[WaterPhysicsBarrier] Cannot initialize with non-positive grid dimensions.");
            return false;
        }
    ...
    internal WaterPhysicsBarrier Clone()
    {
        WaterPhysicsBarrier clone = new WaterPhysicsBarrier();
        ...
        string axis)
    {
        ...
        {
            Debug.LogWarning($"[WaterPhysicsBarrier] Cannot set {axis}-edge barrier outside the initialized grid.");
            return false;
        }

        if (!IsFinitePositive(height) || !IsFiniteNonNegative(seepage))
        {
            Debug.LogWarning("[WaterPhysicsBarrier] Barrier height must be finite and positive; seepage must be finite and non-negative.");
            return false;
        }
        ...
        string axis)
    {
        if (!IsInBounds(simX, simY))
        {
            Debug.LogWarning($"[WaterPhysicsBarrier] Cannot clear {axis}-edge barrier outside the initialized grid.");
            return false;
        }
