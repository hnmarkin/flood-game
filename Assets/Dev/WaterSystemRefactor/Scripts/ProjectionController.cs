...
public sealed class ProjectionController : MonoBehaviour
{
    ...
    public event System.Action<WaterProjection> OnForecastReplaced;
    public WaterProjection CurrentForecast { get; private set; }
    ...
    {
        if (waterController == null || !waterController.TryBuildProjection(forecastSimulatedDuration, out WaterProjection forecast))
            return false;
    ...
    {
        Debug.LogWarning(
            "[ProjectionController] CalculateHazards is a deliberate placeholder. " +
    ...
    private void OnWaterStepped(WaterStepSummary _)
    {
        MarkForecastDirty();
    }
