# Refactored Water System: Status and Future Work

The core water-system refactor chapter is complete: the `RefactorScene` prototype initializes, renders, and advances water through the new runtime seam, and the automated EditMode and PlayMode suites pass. The refactor is not yet a replacement for the Legacy system: the playable scene still uses `WaterSimulator`, and several integrations depend on Game State, Modifiers, Scenario Data, and the New Input System. Those dependencies are future work, not missing prototype functionality.

## Future Work

These items should be revisited when their dependent systems exist:

- Connect the new controller to the production scenario/data pipeline, UI, Game Flow, Game Phase, modifiers, input, and defense-placement systems.
- Test the editor converter against the real legacy `TileMapData` used by the playable map, then run the converted map through the new controller and renderer.
- Verify converter fidelity for coordinates, sparse cells, elevations, initial water, terrain behavior, visual bands, and repeated conversion; keep generated assets in a dedicated Dev conversion folder.
- Isolate the Legacy water runtime behind the conversion seam, replace playable references, and remove/archive the Legacy runtime after cutover.
- Validate one production map for visual parity, source/boundary behavior, barriers, tuning, and performance.
- Update the water-system README and architecture documentation to describe the new runtime and the Legacy converter role.

## At a Glance

| Work item | Parity or refactor change | Legacy system | Refactor status and remaining dependency |
|---|---|---|---|
| Production integration | Parity | `PlayableTilemap` is wired to `WaterSimulator`. | Wire the playable/scenario path and UI to `WaterController`, then retire the Legacy runtime path. Depends on Scenario and Content Data. |
...
| Simulation behavior | Parity validation | Shallow-water flow and spread logic exist inside `WaterSimulator`. | Flow runs in `WaterPhysics`. Compare intended behavior, boundaries, and tuning before integration; defer stress testing until the frame is complete. |
| Barriers | Parity, with decoupling | Uses `IBarrierProvider` through an arbitrary serialized `MonoBehaviour`. | `WaterPhysicsBarrier` is the concrete runtime barrier store. Connect real defense placement and verify blocked-flow/seepage behavior when those systems are ready. |
| Rendering | Parity, with decoupling | `TileMapData`, `TileManager`, and `DynamicTile` update visuals from the Legacy data model. | `WaterRenderer` reads runtime state and new visual definitions. Finish production visual definitions and integration. |
...
| Authoritative runtime state | Refactor-only decoupling | Water is split between `TileMapData`, simulation arrays, tile instances, and visual arrays. | `WaterState` is authoritative and private behind the controller. Preserve this boundary during integration. |
...
Legacy map loading uses `MapLoader` to populate `TileMapData` with `TileInstance` objects. `WaterSimulator` then reads and writes the same data while `TileManager` refreshes the display. The refactor’s converter, `MapDef`, accessor, runtime state, and renderer replace that path; the remaining work is connecting the new assets to the eventual Scenario and Content Data pipeline and validating the conversion of production maps.
...
Legacy already has a barrier hook through `IBarrierProvider`, but it accepts any `MonoBehaviour` and casts it at runtime. The refactor deliberately removes that swappable compatibility layer and uses `WaterPhysicsBarrier` as the final runtime representation. The remaining work is feeding that grid from the eventual defense-placement system and checking the intended blocked-flow and seepage rules.

Legacy rendering is coupled to `TileMapData` and includes separate visual water arrays. The refactor renderer reads `WaterState` and writes only to the configured tilemaps, so the remaining parity work is production tile/visual setup rather than another state path.
...
- **Authoritative state and separation of concerns:** `WaterState`, `WaterPhysics`, `WaterController`, and `WaterRenderer` separate mutable simulation state, calculation, public access, and display. Do not reintroduce Legacy runtime references or public state access.
