...
- `WaterState` remains authoritative for one live water run and stays private behind `WaterController`. Legacy water runtime types remain editor conversion inputs only.
...
- Water has no knowledge of defense objects or construction stages. A completed tile modifier changes runtime terrain through a validated controller operation. A completed scenario modifier reaches water through the resolved modifier snapshot. `WaterPhysicsBarrier` is used only for a completed defense deliberately modeled as an edge-flow barrier with physical properties such as height and seepage.
