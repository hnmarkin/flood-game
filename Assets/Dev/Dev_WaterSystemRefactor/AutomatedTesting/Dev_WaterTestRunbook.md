...
Open **Window > General > Test Runner**. Run the `Dev_WaterSystemRefactor.EditModeTests` assembly in EditMode, then `Dev_WaterSystemRefactor.PlayModeTests` in PlayMode. Deterministic unit and scenario tests must appear only in EditMode; lifecycle and rendering tests must appear only in PlayMode.
