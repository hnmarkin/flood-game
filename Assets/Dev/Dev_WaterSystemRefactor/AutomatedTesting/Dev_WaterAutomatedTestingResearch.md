...
| --- | --- | --- |
| Map access and coordinate conversion | `Dev_MapDef`, `Dev_MapCellDef`, `Dev_MapAccessor` | EditMode unit |
| Mutable state and dirty tracking | `Dev_WaterState` | EditMode unit |
| Numerical settings, sources, modifiers, summaries, projections | `Dev_WaterTypes`, `Dev_WaterProjection` | EditMode unit |
| Flow, sources, drainage, barriers, active-region gating | `Dev_WaterPhysics`, `Dev_WaterPhysicsBarrier` | EditMode unit/scenario |
| Scenario validation and cloning | `Dev_ScenarioDef`, `Dev_WaterStormProfile` | EditMode unit |
| Public orchestration and profile changes | `Dev_WaterController` | EditMode where possible; PlayMode for Unity lifecycle |
| Game-flow/phase callbacks and one-time preliminary flooding | `Dev_WaterLifecycleCoordinator` | PlayMode integration |
| Tilemap writes and renderer lifecycle | `Dev_WaterRenderer`, `Dev_RendererDef` | EditMode band-resolution unit; PlayMode Tilemap integration |
| Projection refresh/event subscription | `Dev_ProjectionController` | PlayMode integration |
...
At review time there are no project `.asmdef` files under `Assets`. That matters because UTF test assemblies need explicit references to the code assembly they test. The clean long-term setup is to give `Assets/Dev/Dev_WaterSystemRefactor/Scripts` a runtime assembly definition, then have test assemblies reference it. Unity documents assembly definitions as a way to control dependencies and reduce unnecessary recompilation: [Organizing scripts into assemblies](https://docs.unity3d.com/6000.0/Documentation/Manual/assembly-definition-files.html).
...
### `Dev_MapAccessorTests` — focused map unit tests
...
### `Dev_WaterStateTests` — state and mutation unit tests
...
### `Dev_WaterTypesTests` and `Dev_ScenarioDefTests` — data/validation unit tests
...
### `Dev_WaterPhysicsTests` — focused physics unit tests
...
- `Dev_WaterStepSummary` reports the correct step index, simulated delta, wet count, total water, maximum depth, and dirty count.

### `Dev_WaterControllerTests` — public orchestration tests
...
### `Dev_WaterLifecycleCoordinatorTests` — lifecycle integration tests
...
- `Dev_RendererDef` resolves dry, shallow, deep, null-band, and boundary-depth cases deterministically.
...
These should be named as scenarios and placed in a `WaterScenario` category, even if their implementation calls `Dev_WaterPhysics` directly. That makes them easy to run as a meaningful group while retaining deterministic unit-test speed.
...
- `Dev_WaterStepSummary` agrees with the state.
...
```text
Assets/Dev/Dev_WaterSystemRefactor/
  Scripts/
    Dev_WaterSystemRuntime.asmdef          # runtime seam for the refactor
  AutomatedTesting/
    EditMode/
      Dev_WaterSystemEditModeTests.asmdef  # NUnit + Unity test references
      Dev_*Tests.cs
    PlayMode/
      Dev_WaterSystemPlayModeTests.asmdef  # NUnit + Unity test references
      Dev_*Tests.cs
```
...
& "<Unity 6000.0.63f1>\Editor\Unity.exe" `
  ...
  -assemblyNames "Dev_WaterSystemEditModeTests" `
  -testResults "Temp\Dev_WaterSystem_EditMode.xml"
```
...
& "<Unity 6000.0.63f1>\Editor\Unity.exe" `
  ...
  -assemblyNames "Dev_WaterSystemPlayModeTests" `
  -testResults "Temp\Dev_WaterSystem_PlayMode.xml"
```

For focused runs, add either `-testFilter "Dev_WaterPhysicsTests"` or a category filter such as `-testCategory "WaterScenario"`. UTF documents `-runTests`, `-batchmode`, `-testPlatform`, `-assemblyNames`, `-testFilter`, `-testCategory`, and `-testResults`: [UTF command-line reference](https://docs.unity.cn/Packages/com.unity.test-framework@1.6/manual/reference-command-line.html). Omit `-testPlatform` only when the default EditMode behavior is wanted.
...
2. Start with `Dev_MapAccessorTests`, `Dev_WaterStateTests`, `Dev_WaterTypesTests`, and validation tests. These should run quickly in EditMode.
